...
public class CartController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if (action.equals("view")) {
			// localhost:8080/ASM/cart?action=view
			doGetViewCart(req, resp);
		} else if (action.equals("add")) {
			// localhost:8080/ASM/cart?action=add&masp={masp}&soluong={soluong}
			String masp = req.getParameter("masp");
			int soluong = Integer.parseInt(req.getParameter("soluong"));
			doGetAddSP(req, resp, session, masp, soluong);
		} else if (action.equals("remove")) {
			// localhost:8080/ASM/cart?action=remove&masp={masp}
			String masp = req.getParameter("masp");
			doGetRemoveSP(req, resp, masp);
		} else if (action.equals("paying")) {
			// localhost:8080/ASM/cart?action=paying&phone={phone}&address={address}
